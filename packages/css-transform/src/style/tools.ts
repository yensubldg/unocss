...
export interface colorObject { [key: string]: string | colorCallback | colorObject }
        ...
        ? Object.getOwnPropertyNames(source).reduce((o, prop) => {
          ...
          if (descriptor) {
            ...
            if (source && typeof source === 'object') {
              o[prop] = deepCopy(
                ((source as unknown) as Record<string, unknown>)[prop],
              )
            }
          }
          return o
...
export function flatColors(colors: colorObject, head?: string): Record<string, string | colorCallback> {
  let flatten: Record<string, string | colorCallback> = {}
...
export function guessClassName(selector: string): { selector: string; isClass: boolean; pseudo?: string } | { selector: string; isClass: boolean; pseudo?: string }[] {
  if (selector.includes(',')) return splitSelectors(selector).map(i => guessClassName(i) as { selector: string; isClass: boolean })
  // not classes, contains attribute selectors, nested selectors - treat as static
  if (selector.charAt(0) !== '.' || searchNotEscape(selector, ['[', '>', '+', '~']) >= 0 || selector.trim().includes(' ') || searchNotEscape(selector.slice(1), '.') >= 0)
